...
//    *filepath_object_shelf1 = "/Users/Jacques/Desktop/Concordia/Concordia Notes/Session Summer 2017 1/COMP 371/Files/OpenGLWorkspace/Models/shelves/lpm7bqz54jr4-Dresser/Dresser.3ds";
/*
    It's difficult to find a model file that abides by the constraints of the object loader (both mine and yours). Information is often missing from the .mtl files provided to us by the 3D object creators. In particular, normal vector coordinates are pretty frequently absent. Wonder if there wouldn't be a way to bypass that? Elsewise, we're just gonna be hunting for objects that happen to have all the info we need (which is to say vertices, tangents, bitangents (not 100% about tangents and bitangents, but the rest are definitely required), normals, and texture coordinates and files (.png, .jpeg files) information.
    I'm gonna try downloading entire scenes to see if I can't just export little segments of them (i.e. their chairs and other furniture), because they're bound to have all this info set up. I already had some success with this method, but it was short-lived; there were no texture coordinates or files. Literally everything else was there. *sigh*
 */
#endif
...
//Additions for moving targets
glm::mat4 moveable_target;//see set_moveable(const unsigned int)
GLfloat moveable_target_velocity = 0.05f;
vector<glm::mat4> targets_translation_vector;
enum Targets{NANOSUIT = 0, WHATEVER_ELSE_IN_SCENE};
...
void do_movement();
//void processInput(); to be defined
void set_moveable(const unsigned int target_number);
...
//        set_moveable(NANOSUIT);
//        model_matrix = targets_translation_vector[NANOSUIT];//uncomment this and the program fails, don't know why
        /*
         I'd really like to have some sort of super-sexy function here like set_moveable, where we assign an unsigned int that communicates an index from the vector<glm::mat4> targets_translation_vector, and from that index we can manipulate the translation matrix of the object in question and then just pass it directly to the target; would one such functionality be possible?
         */
...
//A function to be passed a model matrix, to be modified by user input. This will be to help us determine where stuff (furniture) should go within the house
//Know of a way to make this possible? At present, we're only taking the local value of the model matrix, as opposed to affecting changes to the actual variable
//This doesn't work for some reason! :/
void set_moveable(const unsigned int target_number)
{
    if (keys[GLFW_KEY_UP])//Z-
    {
        targets_translation_vector[target_number] =
            glm::translate(targets_translation_vector[target_number], glm::vec3(0.0f, 0.0f, -moveable_target_velocity));
    }
    if (keys[GLFW_KEY_DOWN])//Z+
    {
        targets_translation_vector[target_number] =
            glm::translate(targets_translation_vector[target_number], glm::vec3(0.0f, 0.0f, moveable_target_velocity));
    }
    if (keys[GLFW_KEY_RIGHT])//X+
    {
        targets_translation_vector[target_number] =
            glm::translate(targets_translation_vector[target_number], glm::vec3(moveable_target_velocity, 0.0f, 0.0f));
    }
    if (keys[GLFW_KEY_UP])//X-
    {
        targets_translation_vector[target_number] =
            glm::translate(targets_translation_vector[target_number], glm::vec3(-moveable_target_velocity, 0.0f, 0.0f));
    }
    if (keys[GLFW_KEY_RIGHT_SHIFT] && keys[GLFW_KEY_UP])//Y+
    {
        targets_translation_vector[target_number] =
            glm::translate(targets_translation_vector[target_number], glm::vec3(0.0f, moveable_target_velocity, 0.0f));
    }
    if (keys[GLFW_KEY_RIGHT_SHIFT] && keys[GLFW_KEY_DOWN])//Y-
    {
        targets_translation_vector[target_number] =
            glm::translate(targets_translation_vector[target_number], glm::vec3(0.0f, -moveable_target_velocity, 0.0f));
    }
