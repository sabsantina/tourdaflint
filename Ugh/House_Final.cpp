        ...
        for (int index = 0; index < scene_objects.size(); index++)
        {
            ...
            if (index == KITCHEN_LAMP || index == LIVINGROOM_TABLELIGHT || index == BEDROOM_DESK_LAMP)
            {
                switch(index)
                {
                    case KITCHEN_LAMP:
                    {
                        firstLightPos += scene_objects[index].m_position + firstLight_modeloffset;
                        break;
                    }
                    case LIVINGROOM_TABLELIGHT:
                    {
                        secondLightPos += scene_objects[index].m_position + secondLight_modeloffset;
                        break;
                    }
                    case BEDROOM_DESK_LAMP:
                    {
                        thirdLightPos += scene_objects[index].m_position + thirdLight_modeloffset;
                        break;
                    }
                    default://impossible
                    {
...
void transformModelObject(int index)
{
    ...
    if (keys[GLFW_KEY_T])
    {
        ...
        glm::vec3 direction = glm::vec3(scene_objects[index].m_position.x - camera.m_position_point.x,
                                        0.0f,
                                        scene_objects[index].m_position.z - camera.m_position_point.z);
        ...
        {
            new_vector = direction * increaseValue;
        }
        ...
        {
            new_vector = right * (-increaseValue);
        }
        ...
        {
            new_vector = glm::vec3(0.0f, increaseValue, 0.0f);
        }
        ...
        if (falls_within_any_boundingbox(scene_objects[index].m_position + new_vector))
        {
            //...stop the translation
            return;
        }
        //else if the new position leads into a wall...
        else if (wall_collision(scene_objects[index].m_position + new_vector))
        {
            //...stop the translation
            return;
        }
        else
        {
            ...
            scene_objects[index].m_position += new_vector;
            for (int i = 0; i < scene_boundingboxes[index].m_points.size(); i++)
            {
                scene_boundingboxes[index].m_points[i] += new_vector;
            }
        }
    }
