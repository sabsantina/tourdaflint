...
public:
    ...
    void draw(Shader shader)
    {
        ...
        for (unsigned int i = 0; i < m_textures.size(); i++)
        {
            ...
            {
                ss << diffuseNumber++;//transfer number to stream, then increment
            }
            if (texture_name == "texture_specular")
            {
                ss << specularNumber++;//transfer number to stream, then increment
            }
            if (texture_name == "texture_normal")
            {
                ss << normalNumber++;//transfer number to stream, then increment
            }
            if (texture_name == "texture_height")
            {
                ss << heightNumber++;//transfer number to stream, then increment
            }
            number = ss.str();
    ...
    //A function to return a vector filled with the mesh's smallest values for x, y, and z, respectively
    glm::vec3 return_minimumfilled_vector()
    {
        glm::vec3 minimumfilled_vector(0.0f);
        
        for (Vertex v : this->m_vertices)
        {
            if (v.m_position.x < minimumfilled_vector.x)
            {
                minimumfilled_vector.x = v.m_position.x;
            }
            if (v.m_position.y < minimumfilled_vector.y)
            {
                minimumfilled_vector.y = v.m_position.y;
            }
            if (v.m_position.z < minimumfilled_vector.z)
            {
                minimumfilled_vector.z = v.m_position.z;
            }
        }
        //By now, we have all minimum values
        return minimumfilled_vector;
    }
    
    //A function to return a vector filled with the mesh's greatest values for x, y, and z, respectively
    glm::vec3 return_maximumfilled_vector()
    {
        glm::vec3 maximumfilled_vector(0.0f);
        
        for (Vertex v : this->m_vertices)
        {
            if (v.m_position.x > maximumfilled_vector.x)
            {
                maximumfilled_vector.x = v.m_position.x;
            }
            if (v.m_position.y > maximumfilled_vector.y)
            {
                maximumfilled_vector.y = v.m_position.y;
            }
            if (v.m_position.z > maximumfilled_vector.z)
            {
                maximumfilled_vector.z = v.m_position.z;
            }
        }
        //By now, we have all minimum values
        return maximumfilled_vector;
    }
