...
public:
    ...
    //A function to return the x_min, x_max, y_min, y_max, z_min, z_max, respectively.
    vector<GLfloat> return_smallest_and_greatest_values()
    {
        enum Coordinates{X_MIN = 0, X_MAX, Y_MIN, Y_MAX, Z_MIN, Z_MAX};
        
        vector<GLfloat> allvalues;
        glm::vec3 min_values(0.0f);
        glm::vec3 max_values(0.0f);
        for (int index = 0; index < this->m_meshes.size(); index++)
        {
            //Where local_mins is defined as being the smallest values from the mesh in question
            glm::vec3 local_mins = m_meshes[index].return_minimumfilled_vector();
            if (local_mins.x < min_values.x)
            {
                min_values.x = local_mins.x;
            }
            if (local_mins.y < min_values.y)
            {
                min_values.y = local_mins.y;
            }
            if (local_mins.z < min_values.z)
            {
                min_values.z = local_mins.z;
            }
            //Where local_maxes is defined as being the greatest values from the mesh in question
            glm::vec3 local_maxes = m_meshes[index].return_maximumfilled_vector();
            if (local_maxes.x > max_values.x)
            {
                max_values.x = local_maxes.x;
            }
            if (local_maxes.y > max_values.y)
            {
                max_values.y = local_maxes.y;
            }
            if (local_maxes.z > max_values.z)
            {
                max_values.z = local_maxes.z;
            }
        }//end for
        
        allvalues.push_back(min_values.x);
        allvalues.push_back(max_values.x);
        allvalues.push_back(min_values.y);
        allvalues.push_back(max_values.y);
        allvalues.push_back(min_values.z);
        allvalues.push_back(max_values.z);
        return allvalues;
    }
